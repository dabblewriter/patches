...
**A committed change that can never apply is skipped rather than retried forever.** An `ApplyChangesError` from a committed batch first triggers the snapshot reload above — a drifted local replica is the likelier cause, and the reload fixes it losslessly. But a change that fails _again_ after authoritative state was installed has proven the document's history, not this replica, is what can't be applied: every replay across it would throw on every device forever. `PatchesSync` then applies it as a no-op (keeping its revision, so the batch stays contiguous and everything after it lands) — the same resolution the server already uses when it replays committed history via `applyChangesForReconstruction`, so the client converges with server truth instead of diverging from it. The document is still damaged, so a `CommittedPoisonSkippedError` (carrying `docId`, `changeId`, `rev`, and the `ApplyChangesError` as `cause`) is emitted through `onError` once per `(docId, changeId)` for telemetry to collect for repair. OT only — LWW materializes committed ops with `partial` semantics and never strict-throws on them.
