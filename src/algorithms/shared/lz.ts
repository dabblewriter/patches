...
export function compress(uncompressed: string | null): string {
  return _compress(uncompressed, 16, (a: number) => f(a));
}

export function decompress(compressed: string | null): string | null {
  if (compressed == null) return '';
  if (compressed === '') return null;
  return _decompress(compressed.length, 32768, (index: number) => compressed.charCodeAt(index));
}
...
export function compressToUTF16(input: string | null): string {
  if (input == null) return '';
  return _compress(input, 15, (a: number) => f(a + 32)) + ' ';
}
...
export function decompressFromEncodedURIComponent(input: string | null): string | null {
  ...
  return _decompress(normalizedInput.length, 32, (index: number) =>
    getBaseValue(keyStrUriSafe, normalizedInput.charAt(index))
  );
}
...
function _decompress(length: number, resetValue: number, getNextValue: (index: number) => number): string | null {
  ...
  let power: number;
  let c: number | string;
  const data: DecompressData = { val: getNextValue(0), position: resetValue, index: 1 };
