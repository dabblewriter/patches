...
export class PatchesSync extends ReadonlyStoreClass<PatchesSyncState> {
  ...
  protected async syncPendingBranchMetas(): Promise<void> {
    ...
    // Process in order: creates → closes/updates → deletes
    // Creates first so a create+close or create+delete in the same offline session works.
    const creates = pendingBranches.filter(b => b.pendingOp === 'create');
    const closes = pendingBranches.filter(b => b.pendingOp === 'close');
    ...
    for (const branch of creates) {
      ...
      try {
        ...
        const { docId: sourceDocId, branchedAtRev, createdAt: _1, modifiedAt: _2, status: _3, pendingOp: _4, deleted: _5, ...metadata } = branch;
        await branchApi.createBranch(sourceDocId, branchedAtRev, metadata);

        // If the branch was created with status 'closed', close it on the server too
        if (branch.status !== 'open') {
          await branchApi.closeBranch(branch.id);
        }
    ...
    for (const branch of closes) {
      ...
      try {
        ...
        console.error('Failed to sync pending branch close:', branch.id, err);
        this.onError.emit(err instanceof Error ? err : new Error(String(err)));
        break;
      }
    }
    ...
    for (const branch of updates) {
      ...
      try {
        ...
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const { id: _id, docId: _did, branchedAtRev: _bar, createdAt: _ca, modifiedAt: _ma, status: _st, contentStartRev: _csr, pendingOp: _po, deleted: _del, ...metadata } = branch;
